...
optimal_power_rule_w_ts(rnorm(5),rnorm(5,1),c(rnorm(5),rnorm(5,1)),pp,lambda=1e-4)

rsssd = function(x,y,z,delta0,sigma0) cond_power_rule_t_ts(x,y,delta=delta0,target=.80,rob_var=F,maxN=6*length(x))
rsssn = function(x,y,z,delta0,sigma0) cond_power_rule_t_ts(x,y,delta=delta0,target=.80,rob_var=T,type='sn',maxN=6*length(x))
...
rssapprox = function(x,y,z,delta0,sigma0) approximate_power_rule_w_ts(x,y,pp=delta2res(delta=delta0,sigma0,T),lambda=3e-3,maxN=6*length(x))
rsscomb = function(x,y,z,delta0,sigma0) combination_power_rule_w_ts(x,y,pp=delta2res(delta=delta0,sigma0,T),lambda=3e-3,maxN=6*length(x))
rssopt = function(x,y,z,delta0,sigma0) optimal_power_rule_w_ts(x,y,z,pp=delta2res(delta=delta0,sigma0,T),lambda=3e-3,maxN=6*length(x))
...
delta0=1
sigma0=2.8
sapply(condpw <- list(rsssd=rsssd,rsssn=rsssn,rsscond=rsscond,rsspred=rsspred,rssop=rssopt,rssapprox=rssapprox,rsscomb=rsscomb),do.call,args=list(x=x,y=y,z=z,delta0=1,sigma0=2.8))

run_scenario(scenarios[14,],ssrs=condpw)

simulate_scenario(scenarios[10,],ssrs=condpw,B=500)
##run_scenario(scenarios[11,])

options(mc.cores=7)
out <- list()
for(i in c(10:12,16:18)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],ssrs=condpw,B=1e3))
sims <- bind_rows(out)
...
xs <- rnorm(10)
ys <- rnorm(10,.625)
sum(sapply(ys,function(y) sum(xs < y)))
p1 <- sum(outer(ys,xs,FUN=`-`)>0)/100


out <- replicate2(1000,{
    x <- rt(10,4)*.8
    y <- rt(10,4)*.8
    unlist(wilcoxon_ps(x,y+1))})

match_ps <- function(n,shift,scale,dist,B=100){
    rowMeans(replicate2(B,{
        x <- dist(n)*scale
        y <- dist(n)*scale
        unlist(wilcoxon_ps(x,y+shift))}))
}
...
nnorm <- function(d,s,alpha=0.025,beta=.2) {
    ceiling(2*pi * s^2 * (qnorm(alpha,lower=F)+qnorm(beta,lower=F))^2/ (3*d^2))
}

power.w.test <- function(p1,p2,p3,alpha,target){
    pw <- function(n,m,p1,p2,p3,alpha,beta){
        {sqrt(n*m*(n+m+1)/12) * qnorm(alpha) + (n*m-1)/2 - n*m*p1}/
            sqrt(n*m*{p1*(1-p1) + (n-1)*(p2-p1^2) + (m-1)*(p3-p1^2)})
    }
        
    

nnorm(1,1)
power.z.test(1,1,power=.84)
power.u.test(delta2res(1,1))
