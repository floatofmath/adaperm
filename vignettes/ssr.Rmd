...
rsscomb1 = function(x,y,z,delta0,sigma0) combination_power_rule_w_ts(x,y,pp=delta2res(delta=delta0,sigma0,T),lambda=345e-4,maxN=6*length(x))
rsscomb2 = function(x,y,z,delta0,sigma0) combination_power_rule_w_ts(x,y,pp=delta2res(delta=delta0,sigma0,T),lambda=125e-4,maxN=6*length(x))
rsscomb3 = function(x,y,z,delta0,sigma0) combination_power_rule_w_ts(x,y,pp=delta2res(delta=delta0,sigma0,T),lambda=395e-5,maxN=6*length(x))
...
sapply(condpw <- list(rsssd=rsssd,rsssn=rsssn,rsscond=rsscond,rsscomb1=rsscomb1,rsscomb2=rsscomb2,rsscomb3=rsscomb3),do.call,args=list(x=x,y=y,z=z,delta0=1,sigma0=2.8))

options(mc.cores=39)
out <- list()
for(i in c(10,12,16,18,13,15)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],ssrs=condpw,B=1e5))
sims <- bind_rows(out)
