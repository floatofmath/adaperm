...
r_counts <- function(n,theta=1,df=2){
  # if(!is.finite(disp))
  #   rpois(n,lambda=exp(mu)) else
  #   rnbinom(n,mu=exp(mu),size=exp(disp))
  means=rchisq(n=n,df = df)
#   if(is(means,"try-error")) {browser()}
  means[means==0]=.5
  means[means>=10000]=10000
  y=rpois(n = n,lambda = means*theta)
  y
}
...
compare_adaptive_tests_2s(14,28,condpowerrule_counts,control_opts=list(df=5),treatment_opts=list(df=5,theta=1.5),rdist=r_counts,test_statistic=meandiff)
...
#     browser()
    ...
    results <- mclapply2(1:B,function(i){ 
      compare_adaptive_tests_2s(n1=scenario$n1,
                         ...
                         treatment_opts=list(df=scenario$df,theta=scenario$theta))})
    results %>% bind_rows %>% summarize(ASN=mean(ne),maxSN=max(ne),
                                        permtest=mean(permtest),permtest2=mean(permtest2),
                                        invnormT=mean(invnorm),invnormNB=mean(invnorm_nb),
...
sim_counts=run_simulation_counts(scenarios_test,B=1000,rule=condpowerrule_counts,rdist=r_counts,test_statistic=meandiff,resam=1000)

save(sim_counts,file=vfile("../data/sim_counts",ending="rda"))
```

```{r}
library(pander)
load(vfile("../data/sim_counts",ending="rda"))
pander(sim_counts,split.tables=100)
```

See the distribution of the samples ASN and maxSN, is fix. furthermore, I can't decrease n1 and n. 

Wald test is in control under poisson data enerating process, while here it is very anticonservative.
