...
run_scenario <- function(scenario){
    with(scenario,{
             ...
             data_frame(
                 ...
                 adaperm_w=adaperm_DR(c(x,y),rep(0:1,each=nE),n1=n1,n=n,test=ranksum,cer_type=cete,atest_type=ttpe,permutations=10^5))
...
out <- list() 
for(i in 2:4) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=50000))
sims <- bind_rows(out)
#for(i in 1:nrow(scenarios)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=20000))

save(sims,file=vfile('~/adaperm_simresults/null_simulations'))
