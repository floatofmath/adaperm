...
run_scenario <- function(scenario){
    ...
    nE <- combination_power_rule_w_ts(x1,y1,pp=delta2res(delta=delta0,sigma0,T),lambda=lambda,maxN=6*n1,propn=propn)
    ...
    ans <- data_frame(
        ...
        invnorm_w=adaptive_invnorm_wilcoxtest_2s(x[1:nE],y[1:mE],n1=n1,n=n,ne=nE,m1=m1,m=m,me=mE),
        ...
        adaperm_rw=adaperm_DR(c(x[1:nE],y[1:mE]),rep(0:1,c(nE,mE)),n1=n1,n=n,m1=m1,m=m,test=ranksum,cer_type='randomized',atest_type='non-randomized',permutations=10^5),
        adaperm_w=adaperm_DR(c(x[1:nE],y[1:mE]),rep(0:1,c(nE,mE)),n1=n1,n=n,m1=m1,m=m,test=ranksum,cer_type='non-randomized',atest_type='non-randomized',permutations=10^5)
    )
    return(ans)
}
...
scenarios <- data_frame(n1 = rep(c(rep(5,3),rep(50,3),rep(15,3),rep(10,3)),2),
                ...
                sigma0 = rep(rep(c(.80,2.6,1.4,3),each=3),2),
                lambda = rep(c(rep(470e-4,3),rep(580e-5,3),rep(160e-4,3),rep(180e-4,3)),2),
...
## about 3.5h for 1e4
for(i in 4:6) medium_scenarios[[i]] <- bind_cols(scenarios[medium_index[i],],simulate_scenario(scenarios[medium_index[i],],B=1e3))
save(medium_scenarios,file=vfile('~/adaperm_simresults/medium_simulations_newrules'))

for(i in 1:6) large_scenarios[[i]] <- bind_cols(scenarios[large_index[i],],simulate_scenario(scenarios[large_index[i],],B=1e3))
...
load('~/adaperm_simresults/small_simulations_newrules_node6_161006.Rd')
load('~/adaperm_simresults/medium_simulations_newrules_node6_161006.Rd')
load('~/adaperm_simresults/large_simulations_newrules_node6_161007.Rd')
...
latex(ftab,options=list(rownames=FALSE),design='design.txt',file="table.tex")
