...
find_param(function(n0,param,delta) log(rgamma(n0,1.2))*param+delta,10,target=.80,low=.01,up=10)
find_param(function(n0,param,delta) rt(n0,4,ncp=delta*param),10,target=.70)
...
cond_power_rule_t_ts <- function(x,y,delta=1,target=.9,alpha=0.025,maxN=length(x)*6,rob_var=T){
    ...
    var <- ifelse(rob_var,
                  robust_pooled_variance(x,y),
                  pooled_variance(c(x,y),c(rep(0,length(x)),rep(1,length(y)))))
    nE <- power.t.test(delta=delta,sd=sqrt(var),power=target,sig.level=alpha,type="two.sample",alternative="one.sided")$n
    ceiling(min(maxN,nE))
}
...
run_scenario <- function(scenario){
    with(scenario,{
             ...
             delta <- c('normal'=1,
                        ...
                        't4'=parameter,
                        'cont'=1)[distribution]
             nE <- max(n,cond_power_rule_norm_ts(x1,y1,delta=delta,target=.80,rob_var=F))
...
scenarios <- data_frame(n1 = rep(c(rep(5,4),rep(50,4),rep(15,4)),2),
                n =  rep(c(rep(10,4),rep(100,4),rep(30,4)),2),
                distribution = rep(rep(c('normal','log-gamma','t4','cont'),3),2),
                parameter =rep(c(
                    0.80,0.65,0.35,0.65,
                    3.20,0.40,0.35,2.80,
                    1.70,0.75,0.65,1.30),2),
                delta0 = rep(0:1,each=12),
                sigma0 = rep(rep(c(.6,2.5,1.35),each=4),2))
                
## simulate_scenario(scenarios[2,],1000)
simulate_scenario(scenarios[14,],1000)
simulate_scenario(scenarios[16,],1000)
## run_scenario(scenarios[2,])
## run_scenario(scenarios[10,])
out <- list() 
for(i in c(13:16)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=1000))
sims <- bind_rows(out)
