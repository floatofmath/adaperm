...
run_scenario <- function(scenario){
    with(scenario,{
             ...
             n_combs <- choose(2*n1,n1)*choose(2*(n-n1),(n-n1))
                                        #ttpe  <- ifelse(n_combs>10000,'davison_hinkley','midp')
             ttpe <- 'non-randomized'
             cete <- 'non-randomized'
             x <- c(x1,xdist(nE-n1))
             y <- c(y1,ydist(nE-n1))
             ...
             adaperm_t=adaperm_DR(c(x,y),rep(0:1,each=nE),n1=n1,n=n,test=tstat,cer_type=cete,atest_type=ttpe,permutations=20000),
             adaperm_w=adaperm_DR(c(x,y),rep(0:1,each=nE),n1=n1,n=n,test=ranksum,cer_type=cete,atest_type=ttpe,permutations=20000))
...
```{r}
load('~/adaperm_simresults/simulations_node5_160311.Rd')
