...
run_scenario <- function(scenario){
    ...
    nE <- max(n,cond_power_rule_norm_ts(x1,y1,delta=1,target=.80,rob_var=T,maxN=6*n1))
...
out <- list()
for(i in c(10:18)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=10000))
for(i in c(10,13,16)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=1000))
sims <- bind_rows(out)
