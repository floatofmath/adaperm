...
run_scenario <- function(scenario){
    ...
    nE <- combination_power_rule_w_ts(x1,y1,pp=delta2res(delta=delta0,sigma0,T),lambda=n2lambda(n1),maxN=6*length(x))
    ...
    ans <- data_frame(
        ...
        adaperm_rwp=adaperm_DR(c(x,y),rep(0:1,each=nE),n1=n1,n=n,test=ranksum,cer_type='non-randomized',atest_type='non-randomized',permutations=10^5)
    )
    return(ans)
}
...
for(i in c(10,13,16)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=1e4))
sims <- bind_rows(out)
#for(i in 1:nrow(scenarios)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=20000))
print(sims,width=Inf)
save(small_scenarios,medium_scenarios,large_scenarios,file=vfile('~/adaperm_simresults/simulations_newrules'))
