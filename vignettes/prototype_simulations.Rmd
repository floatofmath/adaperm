...
run_scenario <- function(scenario){
    ...
    nE <- combination_power_rule_w_ts(x1,y1,pp=delta2res(delta=delta0,sigma0,T),lambda=lambda,maxN=6*length(x),propn=propn)
    ...
    ans <- data_frame(
        exact=exact,
        nE=nE,
        invnorm_t=adaptive_invnormtest_2s(x,y,n1=n1,n=n,ne=nE,m1=m1,m=m,me=mE),
        invnorm_w=adaptive_invnorm_wilcoxtest_2s(x,y,n1=n1,n=n,ne=nE,m1=m1,m=m,me=mE),
        adaperm_t=adaperm_DR(c(x,y),rep(0:1,c(nE,mE)),n1=n1,n=n,m1=m1,m=m,test=tstat,cer_type='randomized',atest_type='non-randomized',permutations=10^5),
        adaperm_rw=adaperm_DR(c(x,y),rep(0:1,c(nE,mE)),n1=n1,n=n,m1=m1,m=m,test=ranksum,cer_type='randomized',atest_type='non-randomized',permutations=10^5),
        adaperm_w=adaperm_DR(c(x,y),rep(0:1,c(nE,mE)),n1=n1,n=n,m1=m1,m=m,test=ranksum,cer_type='non-randomized',atest_type='non-randomized',permutations=10^5)
    )
    return(ans)
}
...
scenarios <- data_frame(n1 = rep(c(rep(5,3),rep(50,3),rep(15,3),rep(10,3)),2),
                ...
                parameter =rep(c(
                    ...
                    1.5,1.25,1.35,
                    3,2.5,2.7),2),
                delta0 = c(rep(0,12),rep(1,9),rep(2,3)),
                sigma0 = rep(rep(c(.80,2.6,1.5,3),each=3),2),
                lambda = rep(c(rep(450e-4,3),rep(550e-5,3),rep(155e-4,3),rep(145e-4,3)),2),
                propn = rep(c(rep(1/2,9),rep(1/3,3)),2))


##simulate_scenario(scenarios[24,],100)
##run_scenario(scenarios[24,])
...
## about 1h for 1e4
for(i in 1:6) small_scenarios[[i]] <- bind_cols(scenarios[small_index[i],],simulate_scenario(scenarios[small_index[i],],B=1e4))
...
for(i in 1:6) example_scenarios[[i]] <- bind_cols(scenarios[example_index[i],],simulate_scenario(scenarios[example_index[i],],B=1e4))


sims <- bind_rows(out)
#for(i in 1:nrow(scenarios)) out[[i]] <- bind_cols(scenarios[i,],simulate_scenario(scenarios[i,],B=20000))
print(sims,width=Inf)
save(small_scenarios,file=vfile('~/adaperm_simresults/small_simulations_newrules'))
save(medium_scenarios,file=vfile('~/adaperm_simresults/medium_simulations_newrules'))
save(large_scenarios,file=vfile('~/adaperm_simresults/large_simulations_newrules'))
save(example_scenarios,file=vfile('~/adaperm_simresults/example_simulations_newrules'))
...
load('~/adaperm_simresults/null_simulations_node6_160405.Rd')
small_null_50k <- sims
load('~/adaperm_simresults/alt_simulations_node2_160405.Rd')
small_alt_50k <- sims
load('~/adaperm_simresults/null_simulations_large_node6_160412.Rd')
large_null_50k <- sims
load('~/adaperm_simresults/alt_simulations_large_node6_160419.Rd')
large_alt_50k <- sims
load('~/adaperm_simresults/simulations_medium_node6_160429.Rd')
