...
cond_power_rule_norm_ts <- function(x,y,delta=1,target=.9,alpha=0.025,maxN=length(x)*6,rob_var=T){
    n1 <- length(x)
    var <- ifelse(rob_var,
...
out <- replicate(1000,{
    ...
    print(rnE <- cond_power_rule_norm_ts(x[1:8],y[1:8],delta=6,target=.8,rob_var=T))
    print(nE <- cond_power_rule_norm_ts(x[1:8],y[1:8],delta=6,target=.8,rob_var=F))
    c(rnE=rnE,nE=nE,rpv=robust_pooled_variance(x[1:8],y[1:8]),pv=pooled_variance(c(x[1:8],y[1:8]),rep(0:1,each=8)))})
## nE should be 28, var 64
rowMeans(out)

power.z.test <- function(delta,sigma,target,alpha=.025) 2*(qnorm(alpha,lower=F)+ qnorm(target))^2*(sigma/delta)^2

print(n0 <- ceiling(power.z.test(6,6,.8)))
...
adaperm(c(y,yE,x,xE),g=rep(0:1,each=nE),8,16,test_statistic=meandiff,cer_type='randomized',atest_type='non-randomized')
adaperm(c(y,yE,x,xE),g=rep(0:1,each=nE),8,16,test_statistic=maritzm,cer_type='randomized',atest_type='non-randomized')
adaptive_invnormtest_2s(c(y,yE),c(x,xE),8,16,nE)
adaptive_invnorm_wilcoxtest_2s(c(y,yE),c(x,xE),8,16,nE)

## First stage t-test p-value
t.test(y[1:8],x[1:8],alternative='less',var.equal=T,conf.level=.975)
t.test(c(y[9:16],yE),c(x[9:16],xE),alternative='less',var.equal=T,conf.level=.975)

## compute the p-values and critical boundaries for the combination test using addplan or east
##install.packages('AGSDest')
## library(AGSDest)
## gsd <- plan.GST(K=2,SF=1,alpha=.025,delta=1,pow=.8,phi=0)
## plot(gsd)
## this is useless
...
bisect <- function(lower,upper,fun,tol=0.0001,...){
    ...
    lambda <- function(lower,upper,fun,flow,fup,...){
        mid  <- (upper + lower)/2
        if(abs(upper - lower) < tol){
            return(mid)
        }
        ...
        lambda(lower,upper,fun,flow,fup,...)
    }
    flow <- fun(lower,...)
    fup <- fun(upper,...)
    if(lower == upper){
